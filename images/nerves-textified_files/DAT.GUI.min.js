...
this.updateDisplay = function() {
    ...
    for (var i = 0; i < options.length; i++) {
      var opt = options[i];
      if (opt.getAttribute('value') == _this.getValue()) {
        opt.selected = true;
      }
	  else {
	  	opt.selected = false;
...
this.prependOption = function( value, selected ) {
  	if( !b.querySelector('option[value="'+value+'"]') ) {
	  	var opt = document.createElement('option');
	    opt.innerHTML = value;
	    opt.setAttribute('value', value);
	   	opt.selected = selected;
		b.insertBefore(opt,b.firstChild);
	}
  };
  
  this.removeOption = function( value ) {
  	var opt = b.querySelector('option[value="'+value+'"]');
	if( opt ) {
  		b.removeChild( opt );
	}
  };
